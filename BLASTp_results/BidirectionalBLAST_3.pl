...
open(ABBLOG, ">AbbreviatedLog_3.log") or die "could not open AbbreviatedLog.log, reason $!";

#have an error log
open(ERRORLOG, ">error_3.log") or die "could not open error.log, reason $!";

#have a dedicated full log
open(FULLLOG, ">FullLog_3.log") or die "could not open FullLog.log, reason $!";
...
open(ECBLASTOUT, ">ECBlastInfo_3.csv") or die "could not create/write ECBlastInfo.csv, reason $!";
...
open(BLASTIN, "<BlastSpecs_3.txt") or die "could not open/read BlastSpecs.txt, necessary file for calibrating this program for the target organism, reason: $!";
