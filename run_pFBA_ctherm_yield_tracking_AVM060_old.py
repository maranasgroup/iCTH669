...
output=open('pFBA_results_ctherm_yield_tracking_AVM060_old.txt','w',buffering=1)
...
#note: running FBA instead of pFBA because for the old model the solver has trouble converging on the minimum flux sum
#alternative, for other objectives and fixed rates
results = object.run(objective,"max",fixed_rates_1) #runs pfba#
...
if results['exception']:
    ...
    cellb_mol_up = -results['EX_cellb_e']['flux']
    ...
    base_bio_yield = results['BIOMASS_CELLOBIOSE']['flux'] * biomass_weight / cellb_mass_up
    base_etoh_yield = results['EX_etoh_e']['flux'] / cellb_mol_up
    base_ac_yield = results['EX_ac_e']['flux'] / cellb_mol_up
    base_for_yield = results['EX_for_e']['flux'] / cellb_mol_up
    base_lac_yield = results['EX_lac__L_e']['flux'] / cellb_mol_up
    base_pyr_yield = results['EX_pyr_e']['flux'] / cellb_mol_up
    base_mal_yield = results['EX_mal__L_e']['flux'] / cellb_mol_up

    #if here no exception, we have a solution to write about
    #write our results to the output file
    output.write("time to solve: "+str(results['total_time'])+"\n")
    output.write("objective value: "+formatted_string_1+"\n")
    output.write("ethanol maximization reaction flux profile\n\n")
    output.write("reaction\tlb\tlevel\tub\n")
    output.write("----------------------------------------------------------------------------------\n")

    print("objective value: ",formatted_string_1)
...
print("finding max biomass\n")

results = object.run(objective,"max",fixed_rates_2) #runs fba#

max_bio_yield = results['BIOMASS_CELLOBIOSE']['flux'] * biomass_weight / cellb_mass_up
...
model.reactions[model.reactions.index("BIOMASS_CELLOBIOSE")].lower_bound = (in_vivo_yield - 1.96 * in_vivo_sd) / biomass_weight
model.reactions[model.reactions.index("BIOMASS_CELLOBIOSE")].upper_bound = (in_vivo_yield + 1.96 * in_vivo_sd)  / biomass_weight

print("finding min/max ethanol\n")
...
min_etoh_yield = results['EX_etoh_e']['flux'] / cellb_mol_up

print("finding min/max acetate\n")

#find min and max acetate
#set objective
objective = "EX_ac_e"

print("finding min acetate\n")

#run FBA to find the min rate
results = object.run(objective,"min",fixed_rates_2) #runs fba#

min_ac_yield = results['EX_ac_e']['flux'] / cellb_mol_up

print("finding max acetate\n")

#run FBA to find the max rate
#results = object.run(objective,"max",fixed_rates_2) #runs fba#

max_ac_yield = results['EX_ac_e']['flux'] / cellb_mol_up

print("finding min/max formate\n")
...
min_for_yield = results['EX_for_e']['flux'] / cellb_mol_up

#run FBA to find the max rate
results = object.run(objective,"max",fixed_rates_2) #runs fba#

max_for_yield = results['EX_for_e']['flux'] / cellb_mol_up

print("finding min/max lactate\n")

#find min and max lactate
#set objective
objective = "EX_lac__L_e"

#run FBA to find the min rate
results = object.run(objective,"min",fixed_rates_2) #runs fba#

min_lac_yield = results['EX_lac__L_e']['flux'] / cellb_mol_up

#run FBA to find the max rate
results = object.run(objective,"max",fixed_rates_2) #runs fba#

max_lac_yield = results['EX_lac__L_e']['flux'] / cellb_mol_up

print("finding min/max pyruvate\n")
...
min_pyr_yield = results['EX_pyr_e']['flux'] / cellb_mol_up

#run FBA to find the max rate
results = object.run(objective,"max",fixed_rates_2) #runs fba#

max_pyr_yield = results['EX_pyr_e']['flux'] / cellb_mol_up

print("finding min/max malate\n")

#find min and max malate
#set objective
objective = "EX_mal__L_e"

#run FBA to find the min rate
results = object.run(objective,"min",fixed_rates_2) #runs fba#

min_mal_yield = results['EX_mal__L_e']['flux'] / cellb_mol_up

#run FBA to find the max rate
results = object.run(objective,"max",fixed_rates_2) #runs fba#

max_mal_yield = results['EX_mal__L_e']['flux'] / cellb_mol_up
